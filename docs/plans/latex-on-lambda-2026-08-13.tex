...
LaTeX-generated documents\index{LaTeX} are currently produced by a natively-installed \texttt{lualatex}/\texttt{latexmk} runtime that lives on the client's single production server (32 GB RAM, running nginx + PHP + MySQL + the LaTeX stack installed directly on Ubuntu). For the current \textbf{commercial-only} order volume (\textasciitilde 7 orders/day), the monolithic setup is comfortable\index{monolith}.
...
\begin{itemize}[nosep]
  ...
  \item LaTeX toolchain (\texttt{lualatex} + \texttt{latexmk} + supporting binaries) installed natively on Ubuntu
...
Order volume from the past three days: 8, 7, 6 --- a running average of \textbf{7 orders/day}\index{commercial volume}. Peak simultaneous LaTeX generations rarely exceeds one at a time. The LaTeX process has all the memory it wants because nothing else is asking.
...
Custom container image (up to 10 GB) that contains TeX Live, invoked per-document via SQS trigger or synchronous invoke. Auto-scales to concurrency, zero infrastructure when idle. See Appendix~\ref{app:lambda}.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Shadow mode}: enqueue jobs from the current monolith but continue serving the in-band production output; compare outputs
  ...
  \item \textbf{Retire the in-monolith LaTeX path}
\end{enumerate}

\subsection{Risks common to all options}
\begin{itemize}[nosep]
  \item \textbf{Template drift}: the current production install's TeX Live + package versions become the accidental reference; some templates may rely on specific package versions
...
Four attendees:
\begin{itemize}[nosep]
  \item \textbf{Zac Hudson} --- client Drupal developer; owner of the current LaTeX generation code; extended and maintains the \texttt{Document Service latex Generator}. Runs against a native LaTeX stack installed directly on the Ubuntu production instance (not Dockerized --- Zac replaced the original Docker path Kurt built with an in-place install to avoid script-plumbing overhead).
...
\begin{center}
\begin{tabular}{@{}p{3.6cm}p{1.6cm}p{8.3cm}@{}}
\toprule
\textbf{Parameter} & \textbf{Required?} & \textbf{Purpose} \\
\midrule
\texttt{input\_zip\_url} & required & S3 URL of the input zip that Drupal has built and uploaded. Contains everything the Lambda needs to compile: main \texttt{.tex} file, \texttt{.sty} style files, all PDFs to be included, plus any per-report font files not already baked into the container. \\
\texttt{output\_pdf\_url} & required & S3 URL where the finished PDF should be written. Drupal decides the exact path (including any date prefix, tenant prefix, etc.) so name collisions are Drupal's problem, not Lambda's. Also becomes the ``handle'' by which Drupal tracks the job. \\
\texttt{callback\_url} & required & HTTP endpoint Drupal exposes to receive the ``done'' notification when Lambda finishes. Different Drupal modules (report, invoice, future residential) may register different endpoints. \emph{``If you don't tell me how to notify you when I'm done, I don't want to talk to you at all.''} \\
\texttt{runtime\_script\_url} & optional & S3 URL of the runtime script that Lambda should download and execute for this specific run. When absent, Lambda uses the script baked into the container image at build time. Purpose: flexibility --- iterate on the report-generation script without rebuilding the entire Lambda container image. Scripts live in a dedicated scripts bucket (see \S\ref{sec:meeting-storage}). \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tikzpicture}[
  ...
  actor/.style={draw, fill=blue!10, minimum width=1.7cm, minimum height=0.7cm, rounded corners=1pt},
  lifeline/.style={dashed, gray},
  msg/.style={-Stealth, thick},
]
% Actors along the top (6 participants — added S3 Scripts)
\node[actor] (drupal)   at ( 0,   0) {Drupal};
\node[actor] (s3in)     at ( 2.6, 0) {S3 Input};
\node[actor] (s3scr)    at ( 5.2, 0) {S3 Scripts};
\node[actor] (lambda)   at ( 7.8, 0) {Lambda};
\node[actor] (s3prod)   at (10.4, 0) {S3 Prod};
\node[actor] (s3stg)    at (13.0, 0) {S3 Staging};

% Lifelines
\foreach \a in {drupal,s3in,s3scr,lambda,s3prod,s3stg} {
  \draw[lifeline] (\a.south) -- ++(0, -11);
}

% Messages
\draw[msg] (drupal.south |- 0,-0.7)  -- ++(2.6,0)   node[midway,above,font=\scriptsize]{1. upload zip};
\draw[msg] (drupal.south |- 0,-1.5)  -- ++(7.8,0)   node[midway,above,font=\scriptsize]{2. invoke Lambda (input, output, callback, [script])};

\draw[msg] (lambda.south |- 0,-2.3)  -- ++(-5.2,0)  node[midway,above,font=\scriptsize]{3. pull zip};
\draw[msg] (lambda.south |- 0,-3.1)  -- ++(-2.6,0)  node[midway,above,font=\scriptsize,align=center]{4. pull runtime script\\\itshape (or use baked-in default if 4th param absent)};

\node[font=\scriptsize,align=center] at ( 7.8,-4.3) {5. unzip $\rightarrow$ /tmp\\6. QPDF check\\7. Ghostscript (if needed)\\8. execute script (lualatex via latexmk)};
\draw[dashed, gray!60] (6.7, -3.7) rectangle (8.9, -4.9);

\draw[msg] (lambda.south |- 0,-5.5)  -- ++(2.6,0)   node[midway,above,font=\scriptsize]{9. write PDF (canonical, forever)};

\draw[msg] (lambda.south |- 0,-6.3)  -- ++(5.2,0)   node[midway,above,font=\scriptsize]{10. clone to staging (only if large-delivery needed)};

\draw[msg] (lambda.south |- 0,-7.1)  -- ++(-5.2,0)  node[midway,above,font=\scriptsize]{11. delete input zip (self-cleanup)};

\draw[msg] (lambda.south |- 0,-7.9)  -- ++(-7.8,0)  node[midway,above,font=\scriptsize]{12. POST callback (output\_pdf\_url + delivery info)};

\node[font=\scriptsize,align=center,text=gray!70!black] at (7.8,-8.5) {(Lambda container dies)};

\draw[msg] (drupal.south |- 0,-9.3)  -- ++(13.0,0)  node[midway,above,font=\scriptsize]{13. issue presigned URL against staging bucket (Drupal-side, direct SDK)};

\node[font=\scriptsize,align=left,anchor=west] at (-0.5,-10.2) {14. Drupal emails presigned URL to customer (large PDFs) OR attaches directly (small PDFs)};

\end{tikzpicture}
\caption{End-to-end sequence: Drupal $\rightarrow$ Lambda $\rightarrow$ dual-write to production + staging $\rightarrow$ callback $\rightarrow$ time-limited customer delivery. Step 4 fetches the runtime script when the optional 4th parameter is provided; otherwise Lambda uses the script baked into the container image.}
\label{fig:sequence}
\end{figure}
...
Figure~\ref{fig:storage} shows the four-bucket model.

\begin{figure}[h!]
\centering
\begin{tikzpicture}[
  every node/.style={font=\small},
  bucket/.style={draw, fill=orange!15, minimum width=3.4cm, minimum height=1.8cm, rounded corners=2pt, align=center},
  actor/.style={draw, fill=blue!10, minimum width=2.4cm, minimum height=0.7cm, rounded corners=1pt},
  arrow/.style={-Stealth, thick},
]
\node[actor] (drupal) at (-1, 6) {Drupal};
\node[actor] (lambda) at ( 7, 6) {Lambda};
\node[actor] (customer) at (13.5, 2.5) {Customer};

\node[bucket] (input)   at (-1,  3) {\textbf{S3 Input Bucket}\\\footnotesize private, separate\\\footnotesize job zips\\\footnotesize Lambda self-clean\\\footnotesize + 24h lifecycle};
\node[bucket] (scripts) at ( 3,  3) {\textbf{S3 Scripts Bucket}\\\footnotesize private\\\footnotesize runtime scripts\\\footnotesize versioned\\\footnotesize (keep forever)};
\node[bucket] (prod)    at ( 7,  3) {\textbf{S3 Production Bucket}\\\footnotesize private\\\footnotesize canonical PDFs\\\footnotesize (keep forever)};
\node[bucket] (staging) at ( 7, -1) {\textbf{S3 Staging Bucket}\\\footnotesize private\\\footnotesize delivery copies\\\footnotesize (10-day lifecycle)};
...
\draw[arrow] (scripts) -- (lambda) node[midway,above,font=\scriptsize,align=center]{pull script\\\itshape(optional)};
\draw[arrow, dashed] (drupal.east) -- ++(1.5,0) |- (scripts.west) node[near end, above, font=\scriptsize]{publish new scripts};
\draw[arrow] (lambda) -- (prod) node[midway,right,font=\scriptsize]{write canonical};
\draw[arrow] (lambda.south) -- ++(0,-1) -| (staging) node[near end, right,font=\scriptsize]{clone if needed};
\draw[arrow, dashed] (staging) -- (customer) node[midway, above, align=center, font=\scriptsize\itshape]{presigned URL\\(7 day max)};

\end{tikzpicture}
\caption{Four-bucket storage model. Input holds job payloads (short-lived). Scripts holds runtime scripts that Lambda can optionally pull for per-invocation flexibility (long-lived, versioned by convention). Production keeps the canonical PDF forever. Staging receives a copy only when large-file delivery via presigned URL is required; objects auto-expire after 10 days, presigned URLs valid for max 7 days --- URL always expires before the underlying object. Scripts kept in a separate bucket so its long-lived nature doesn't pollute the short-lived buckets and vice versa.}
\label{fig:storage}
\end{figure}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Invoices} (already LaTeX-generated in Zac's current native Ubuntu install)
...
\begin{itemize}[nosep]
  \item Send Kurt the path to the current \texttt{Document Service latex Generator} implementation
  \item Send Kurt the current Drupal invocation points (where the code today calls into the LaTeX pipeline) --- Kurt uses this to build the patches that Drupal will need to redirect calls at Lambda
  \item Design the callback endpoint schema (payload shape, HTTP verbs, error semantics)
  \item Design the Drupal-side submission flow: build zip, upload to S3 input bucket, invoke Lambda with three required params and one optional param
  \item Add outstanding-jobs tracking to Drupal (a small table; up to $\sim$1000 rows expected under residential load)
  \item Get Kurt a list of fonts that need to be baked into the Lambda container image
  \item Start work \emph{next week} (2026-08-18 or later)
\end{itemize}

\subsubsection{WorxCo's TODO}
\begin{itemize}[nosep]
  \item Build the Lambda container image (fresh, not from MVP): TeX Live 2025+, \texttt{lualatex}, \texttt{latexmk}, QPDF, Ghostscript, preloaded fonts, entrypoint script, and a baked-in default runtime script (used when the 4th param is absent)
  \item Build the ECR-container Image Builder pipeline with monthly + on-demand triggers
  \item Provision the four S3 buckets: input, production PDFs, staging (10-day lifecycle), scripts
  \item Build the Lambda function + invocation-URL infrastructure that accepts the four-parameter call (three required + one optional \texttt{runtime\_script\_url})
  \item Provide Zac the Lambda invocation spec (HTTP endpoint URL, expected parameters, callback contract)
  \item Grant Drupal's PHP-FPM instance role the IAM permissions it needs on each of the four buckets (see \S\ref{sec:meeting-presigned-url} for the concrete policy) --- least-privilege at the PHP-FPM tier, no static keys handed to Drupal
\end{itemize}
...
\subsubsection{IAM permissions Drupal's PHP-FPM role needs}

Drupal touches all four buckets. Capability matrix (Kurt's spec, translated to concrete IAM actions):

\begin{center}
\begin{tabular}{@{}p{2.4cm}p{11cm}@{}}
\toprule
\textbf{Bucket} & \textbf{Actions} \\
\midrule
Input      & \texttt{s3:PutObject}, \texttt{s3:GetObject}, \texttt{s3:GetObjectAttributes}, \texttt{s3:ListBucket} \\
Production & \texttt{s3:PutObject}, \texttt{s3:GetObject}, \texttt{s3:GetObjectAttributes}, \texttt{s3:ListBucket} \\
Staging    & \texttt{s3:PutObject}, \texttt{s3:GetObject}, \texttt{s3:GetObjectAttributes}, \texttt{s3:DeleteObject}, \texttt{s3:ListBucket} \\
Scripts    & \texttt{s3:PutObject}, \texttt{s3:GetObject}, \texttt{s3:GetObjectAttributes}, \texttt{s3:ListBucket} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{On the ``readmeta'' action}: this maps to \texttt{s3:GetObjectAttributes}, the modern AWS API for reading object metadata without downloading the object body. Drupal (via the AWS PHP SDK / flysystem) frequently calls this before actual reads to check existence or size. Historic incident: a few weeks back Drupal failed to read an S3 object because the role had \texttt{GetObject} but not the newer \texttt{GetObjectAttributes} --- Drupal's pre-read metadata check tripped first. Granting both closes that gap.

\textbf{On generating presigned URLs}: no separate IAM action. \texttt{createPresignedRequest} is client-side HMAC math. The resulting URL delegates \texttt{GetObject} to the URL holder, so as long as the SIGNER's identity has \texttt{s3:GetObject} on the bucket, the URL will work.
...
\textbf{Lambda's IAM role is separate.} Lambda needs: read from Input + Scripts, write to Production + Staging, delete from Input (for self-cleanup). Not included in this policy --- Lambda's execution role is provisioned separately with its own least-privilege policy.

\textbf{On least-privilege refinement}: the actions above are broader than strictly needed today (e.g., Drupal doesn't strictly need to write to Production if Lambda is the only writer). Kurt's rationale: catch missing permissions early during integration rather than debug obscure ``access denied'' errors mid-development. Tighten later once Drupal's actual call surface is stable.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Migration status of LaTeX in production}: production still runs Zac's native-Ubuntu LaTeX pipeline; the sandbox rebuild did not migrate that. Zac needs to identify all the Drupal call sites so Kurt can prepare the patches that redirect them at Lambda.
\end{itemize}
...
At the time, the residential pivot was a lower priority for the client. Commercial order volume (7 orders/day) didn't stress the existing monolith LaTeX path. Rather than integrate the MVP into production for a workload that didn't need it, the team parked it.
