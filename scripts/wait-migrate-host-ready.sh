...
#
# wait-migrate-host-ready: block until the migrate-host EC2 instance has
# actually finished its bootstrap, not just "the CloudFormation API call
# returned successfully."
#
# Why this exists: `make deploy-migrate-host` finishes when CFN reports
# CREATE_COMPLETE, which only means the AWS resource was provisioned —
# the EC2 instance's UserData (which runs scripts/migrate-host/bootstrap.sh)
...
# causes downstream commands (dispatch-restore-mysql, dispatch-run-pgloader) to fail
# because migrate-host isn't actually ready to take SSM dispatches yet.
#
# Two-stage wait:
...
#   3. Sanity-check /etc/worxco/migrate-host-marker exists (the marker
#      bootstrap.sh writes near its end)
#
# Usage: scripts/wait-migrate-host-ready.sh

set -euo pipefail

INSTANCE_ID=$(aws ec2 describe-instances \
  --filters "Name=tag:Name,Values=cf-migrate-host" \
            "Name=instance-state-name,Values=running" \
  --query 'Reservations[0].Instances[0].InstanceId' --output text 2>/dev/null || true)

if [ -z "$INSTANCE_ID" ] || [ "$INSTANCE_ID" = "None" ]; then
  echo "ERROR: cf-migrate-host EC2 instance not running" >&2
  exit 1
fi
echo "Migrate-host: $INSTANCE_ID"
...
# ============================================================
echo "Stage 3/3: Verifying /etc/worxco/migrate-host-marker exists..."
MARKER_CMD=$(aws ssm send-command \
  --instance-ids "$INSTANCE_ID" \
  --document-name "AWS-RunShellScript" \
  --parameters 'commands=["test -f /etc/worxco/migrate-host-marker && echo MARKER_OK || echo MARKER_MISSING"]' \
  --query 'Command.CommandId' --output text)
sleep 3
...
if echo "$MARKER_OUT" | grep -q "MARKER_OK"; then
  ...
  echo "ERROR: cloud-init finished but /etc/worxco/migrate-host-marker is missing." >&2
  echo "       bootstrap.sh likely failed partway. Check /var/log/cloud-init-output.log on the instance." >&2
  exit 1
fi

echo ""
echo "✓ Migrate host is FULLY READY (SSM online, cloud-init done, marker present)"
