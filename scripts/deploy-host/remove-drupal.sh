...
log "  - $PRIVATE_DIR (hash_salt file — kept during file→SSM transition; SSM is primary)"
...
log "  - Secrets Manager: worxco/$ENV/drupal/hash-salt (primary source of hash_salt)"
log ""
...
log "  aws secretsmanager delete-secret --secret-id worxco/$ENV/drupal/hash-salt --force-delete-without-recovery"
