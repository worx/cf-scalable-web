...
# NOTE: DNS + ALB warm-up moved to scripts/publish-dns.sh (end of file).
# Rationale: install-drupal runs BEFORE publish-dns in the standard
# install-drupal-full pipeline, so any warm-up here fires against the
# PRIOR DNS entry (or NXDOMAIN if it's a cold-start env), not the fresh
# alias publish-dns is about to write. publish-dns is the natural home
# for a warm-up — the resolver flush runs right after the record is
# authoritatively INSYNC. See publish-dns.sh's "Warm-up" section.
