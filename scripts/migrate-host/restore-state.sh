...
#
# Purpose:  Restore migrate-host operator-managed state from the sandbox
#           migrate-host backups S3 bucket. Includes a safety-net
#           snapshot to /tmp before extracting, and writes stub
#           .aws/credentials files pointing back to the Mac for the
#           real credentials.
#
# Two invocation modes:
#   1. Manual on-demand via `make restore-migrate-host`
#   2. Automatic in UserData at first-boot of a new migrate-host
#
...
#
# Credentials stub (migrate-host — same pattern as deploy-host):
...
#   MIGRATE_HOST_BACKUPS_BUCKET   Bucket    (sandbox-migrate-host-backups-kv-worxco)
#   S3_PREFIX                    Top-level (config)
#   RESTORE_KEY                  Object key (<prefix>/migrate-host-latest.tar.gz)
...
#
# Runs as:  root on migrate-host
# Host:     migrate-host (sandbox VPC, project VPC public subnet)
# Called by:
#   - `make restore-migrate-host` (on-demand)
#   - UserData at first boot (via cf-migrate-host.yaml)
#
# Undo:
...
if [ -f "$SCRIPT_DIR/_common.sh" ]; then
  ...
  # SSM-uploaded layout: _common.sh at /opt/migrate-host/scripts/
  # shellcheck disable=SC1091
  source "$SCRIPT_DIR/../_common.sh"
...
RESTORE_KEY="${RESTORE_KEY:-$S3_PREFIX/migrate-host-latest.tar.gz}"
...
# ============================================================
log_init "restore-state-migrate-host"
trap 'log_upload_and_exit "$MIGRATE_HOST_BACKUPS_BUCKET"' EXIT

log_step "restore-state (migrate-host) - sandbox S3 → migrate-host state"
...
require_env MIGRATE_HOST_BACKUPS_BUCKET RESTORE_KEY
...
if is_dry_run; then
  log_info "[DRY_RUN] would head-object s3://$MIGRATE_HOST_BACKUPS_BUCKET/$RESTORE_KEY"
elif ! aws s3api head-object \
       --bucket "$MIGRATE_HOST_BACKUPS_BUCKET" --key "$RESTORE_KEY" >/dev/null 2>&1; then
  log_error "Source archive does not exist: s3://$MIGRATE_HOST_BACKUPS_BUCKET/$RESTORE_KEY"
  log_error "  On a fresh migrate-host with no prior backup, this is EXPECTED - skip restore."
  exit 1
fi
...
if is_dry_run; then
  log_info "[DRY_RUN] would run: aws s3 cp s3://$MIGRATE_HOST_BACKUPS_BUCKET/$RESTORE_KEY $DOWNLOAD_PATH"
else
  log_info "Downloading s3://$MIGRATE_HOST_BACKUPS_BUCKET/$RESTORE_KEY"
  if ! aws s3 cp "s3://$MIGRATE_HOST_BACKUPS_BUCKET/$RESTORE_KEY" "$DOWNLOAD_PATH" --quiet; then
...
# ============================================================
if is_dry_run; then
  log_step "restore-state (migrate-host) - DRY RUN complete (no changes made)"
  log_info "Would have restored ${#KNOWN_PATHS[@]} tracked path locations from"
  log_info "  s3://$MIGRATE_HOST_BACKUPS_BUCKET/$RESTORE_KEY"
else
  log_step "restore-state (migrate-host) complete"
  log_ok "Deploy-host state restored from s3://$MIGRATE_HOST_BACKUPS_BUCKET/$RESTORE_KEY"
