...
#
# Purpose:  Back up migrate-host operator-managed state (SSH keys,
#           .env, .aws/config, custom bin/ dirs) to the sandbox
#           migrate-host backups S3 bucket.
#
#           Mirrors scripts/deploy-host/backup-state.sh
#           but targets migrate-host paths and its own dedicated bucket
...
#     /root/.env, /root/bin/     Env vars + custom scripts for root
#     /home/ubuntu/.env, /home/ubuntu/bin/  Same for ubuntu
#
#   AWS CLI configuration (config only - NOT credentials):
#     /root/.aws/config
...
#
#   Shell + tool history (operator memory - what did I do last week?):
#     /root/.zsh_history, /root/.mysql_history
...
#   /root/.zshrc, /home/ubuntu/.zshrc
#     - Managed by scripts/deploy-host/bootstrap.sh (rewritten on every boot)
#   .cache/*, .local/share/composer, .npm/, .zcompdump, .viminfo, .lesshst
#     - Regenerable package caches or ephemeral runtime state
#   Git repo (~/projects/...), FSx mounts, MariaDB
#     - Handled by deploy-host CFN or persistent storage
#
# Note on shell/mysql history files: may contain sensitive query text
# (passwords in historical CREATE USER statements, secret values in old
# shell commands). They ride in the same backup as SSH private keys -
# consistent threat model. Bucket has AES256 encryption at rest and
# public access fully blocked.
#
# S3 layout produced:
#   s3://$MIGRATE_HOST_BACKUPS_BUCKET/config/migrate-host-archive/migrate-host-<UTC>.tar.gz
#     └── timestamped durable copy; 90-day expiration via bucket lifecycle
#   s3://$MIGRATE_HOST_BACKUPS_BUCKET/config/migrate-host-latest.tar.gz
#     └── S3-side copy of the newest timestamped archive; never expires;
#         auto-restore source for new migrate-host instances
#
...
#   MIGRATE_HOST_BACKUPS_BUCKET  Bucket name  (sandbox-migrate-host-backups-kv-worxco)
#   S3_PREFIX                   Top-level prefix (config)
#   DRY_RUN                     yes = preview commands without executing
#
# Runs as:  root on migrate-host (needs read access to /root and privileged
#           /home/ubuntu subdirs)
# Host:     migrate-host (sandbox VPC, project VPC public subnet)
# Called by:
#   - `make backup-migrate-host` (on-demand from Mac|DH via SSM send-command)
#   - EventBridge Scheduler nightly at 02:00 America/Chicago (via cf-migrate-host.yaml)
#   - `make destroy-migrate-host` (pre-destroy final backup)
#
# Created:  2026-07-15
# ============================================================
...
# ============================================================
MIGRATE_HOST_BACKUPS_BUCKET="${MIGRATE_HOST_BACKUPS_BUCKET:-sandbox-migrate-host-backups-kv-worxco}"
S3_PREFIX="${S3_PREFIX:-config}"

# S3 object keys derived from the prefix
ARCHIVE_PREFIX="$S3_PREFIX/migrate-host-archive"
LATEST_KEY="$S3_PREFIX/migrate-host-latest.tar.gz"
...
# Grouped by category so the "which category do I belong to?" question
# is easy to answer when adding or removing a path.
BACKUP_PATHS=(
  # --- Access / identity ---
  "/root/.ssh"
  "/home/ubuntu/.ssh"
...
# ============================================================
log_init "backup-state-migrate-host"
trap 'log_upload_and_exit "$MIGRATE_HOST_BACKUPS_BUCKET"' EXIT

log_step "backup-state (migrate-host) - state → sandbox S3"
log_info "MIGRATE_HOST_BACKUPS_BUCKET = $MIGRATE_HOST_BACKUPS_BUCKET"
log_info "S3 destination:"
log_info "  s3://$MIGRATE_HOST_BACKUPS_BUCKET/$ARCHIVE_PREFIX/  ← timestamped (90-day retention)"
log_info "  s3://$MIGRATE_HOST_BACKUPS_BUCKET/$LATEST_KEY       ← latest pointer (never expires)"
if [ "${DRY_RUN:-}" = "yes" ]; then
...
require_env MIGRATE_HOST_BACKUPS_BUCKET
...
ARCHIVE_NAME="migrate-host-${TIMESTAMP}.tar.gz"
...
if is_dry_run; then
  ...
  MANIFEST=$(mktemp)
  {
    echo "# migrate-host state backup manifest"
    ...
    echo "  - .zshrc (managed by bootstrap.sh)"
    echo "  - .cache/, .local/share/composer/, .npm/, .zcompdump (regenerable)"
    echo "  - .viminfo, .lesshst (ephemeral runtime state)"
  } > "$MANIFEST"
...
if is_dry_run; then
  ...
  log_info "  s3://$MIGRATE_HOST_BACKUPS_BUCKET/$TIMESTAMPED_KEY (timestamped, 90-day retention)"
  log_info "  s3://$MIGRATE_HOST_BACKUPS_BUCKET/$LATEST_KEY (latest, never expires)"
else
  # Upload the timestamped archive first - this is the durable copy.
  log_info "Uploading timestamped archive..."
  if ! aws s3 cp "$LOCAL_ARCHIVE" "s3://$MIGRATE_HOST_BACKUPS_BUCKET/$TIMESTAMPED_KEY" --quiet; then
    log_error "Timestamped upload failed"
    rm -f "$LOCAL_ARCHIVE"
    exit 1
  fi
  log_ok "Uploaded: s3://$MIGRATE_HOST_BACKUPS_BUCKET/$TIMESTAMPED_KEY"

  # Then S3-side copy to the latest pointer. Atomic on the S3 side.
  log_info "Updating latest pointer..."
  if ! aws s3 cp \
        "s3://$MIGRATE_HOST_BACKUPS_BUCKET/$TIMESTAMPED_KEY" \
        "s3://$MIGRATE_HOST_BACKUPS_BUCKET/$LATEST_KEY" \
        --quiet; then
  ...
  log_ok "Updated: s3://$MIGRATE_HOST_BACKUPS_BUCKET/$LATEST_KEY"
...
if is_dry_run; then
  ...
  for key in "$TIMESTAMPED_KEY" "$LATEST_KEY"; do
    if S3_SIZE=$(aws s3api head-object \
                   --bucket "$MIGRATE_HOST_BACKUPS_BUCKET" --key "$key" \
                   --query ContentLength --output text 2>/dev/null); then
      log_ok "  s3://$MIGRATE_HOST_BACKUPS_BUCKET/$key ($((S3_SIZE / 1024)) KB)"
    else
      log_error "  s3://$MIGRATE_HOST_BACKUPS_BUCKET/$key NOT FOUND after upload"
      exit 1
    fi
  done
fi
...
# ============================================================
if is_dry_run; then
  log_step "backup-state (migrate-host) - DRY RUN complete (no upload performed)"
  log_info "Would have archived ${#REL_PATHS[@]} paths (~$((TOTAL_BYTES / 1024)) KB)"
  log_info "Would have uploaded to:"
  log_info "  s3://$MIGRATE_HOST_BACKUPS_BUCKET/$TIMESTAMPED_KEY"
  log_info "  s3://$MIGRATE_HOST_BACKUPS_BUCKET/$LATEST_KEY"
else
  log_step "backup-state (migrate-host) complete"
  log_ok "Backup complete: ${#REL_PATHS[@]} paths archived"
  log_info "Restore with: make restore-migrate-host"
  log_info "List backups: make list-migrate-host-backups"
fi
