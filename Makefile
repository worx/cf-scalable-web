...
# Migration workflow lives in its own subdirectory (`migration/`) with a
# standalone Makefile. Run `cd migration && make help` to see targets, or
# delete the entire folder when the client migration is complete.
...
help:  ## Show this help message
	...
	@echo "  make endpoints ENV=...        Show all key endpoints (ALB, RDS, Valkey, FSx) — copy-paste friendly"
	@echo "  make check-drift              Warn when local CFN templates are newer than deployed stacks"
	@echo ""
	...
	@echo "  make publish-drupal-vhost ENV=...  SSM-write drupal.conf to FSx (no full reinstall)"
	...
	@echo "  make smoke-test-drupal ENV=...     Curl the ALB and assert HTTP 200 (DNS-bypassed via --resolve)"
	...
	@echo "  make admin-login-url ENV=...       Generate a one-time drush user:login URL for admin"
	@echo "  make clear-drupal-cache ENV=...    Wipe FSx compiled-container cache + TRUNCATE cache_* tables"
	...
	@echo ""
	@echo "$(YELLOW)Runtime Fleet Operations (ENV=sandbox|staging|production):$(NC)"
	@echo "  make reload-nginx ENV=...          SSM-reload nginx on every nginx box (picks up new vhost configs)"
	@echo "  make restart-php-fpm ENV=...       SSM-restart php-fpm on every PHP box (busts OPcache + env)"
	@echo ""
	@echo "$(YELLOW)FSx Layout (typically auto-invoked by deploy-allX / destroy-all):$(NC)"
	@echo "  make init-fsx-layout ENV=...       Pre-create /fsx/www + /fsx/nginx on a fresh FSx volume"
	@echo "  make unmount-deploy-host-fsx       SSM 'use-env none' to deploy-host (clean umount before destroy-storage)"
	@echo ""
	@echo "$(YELLOW)Admin SSH Key Registry (scp-via-SSM-proxy access):$(NC)"
	@echo "  make admin-ssh-key-add NAME=... PUBKEY=...   Register a public key for SSM-tunneled scp"
	@echo "  make admin-ssh-key-list                      List configured admin keys"
	@echo "  make admin-ssh-key-remove NAME=...           Remove a registered key"
	@echo "  make admin-ssh-key-sync                      Push current registry to running deploy-host"
	@echo ""
	...
	@echo "  make build-amis               Force-rebuild all 3 pipelines (~25 min)"
	@echo "  make build-amis-if-needed     Skip rebuild when AMIs are already at the configured RecipeVersion"
	@echo "  make update-ami-param         Write latest AMI to SSM (PIPELINE=nginx)"
	@echo "  make update-ami-params        Write latest AMIs to SSM for all 3 pipelines"
	@echo ""
	...
	@echo ""
	@echo "$(YELLOW)Prod → Sandbox Migration (one-time client workflow):$(NC)"
	@echo "  $(CYAN)cd migration && make help$(NC)      — enter the migration subfolder for its targets"
	@echo "  $(CYAN)rm -rf migration/$(NC)              — delete when the migration is complete"
	@echo ""
